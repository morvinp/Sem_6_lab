...
// Kernel function to multiply matrices (each element computed by one thread)
__global__ void matrixMultiplyElement(int *A, int *B, int *C, int rowsA, int colsA, int colsB) {
...
int main() {
    ...
    matrixMultiplyElement<<<numBlocks, threadsPerBlock>>>(d_A, d_B, d_C, rowsA, colsA, colsB);
