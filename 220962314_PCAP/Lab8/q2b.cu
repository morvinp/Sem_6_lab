...
// Kernel function to multiply matrices (each column computed by one thread)
__global__ void matrixMultiplyColumn(int *A, int *B, int *C, int rowsA, int colsA, int colsB) {
    int col = threadIdx.x;
    
    // Check if column is valid
    if (col < colsB) {
        // Compute each element of the result column
...
int main() {
    ...
    matrixMultiplyColumn<<<1, colsB>>>(d_A, d_B, d_C, rowsA, colsA, colsB);
